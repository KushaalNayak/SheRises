...
const getAuthToken = () => {
  ...
  // Don't use offline-token as it's not a valid JWT
  if (token === 'offline-token' || !token) {
    return null;
  }
  return token;
};
...
const request = async (endpoint, options = {}) => {
  ...
  const requiresAuth = !endpoint.includes('/auth/register') && 
  ...
  // Check if auth is required but token is missing or invalid
  if (requiresAuth && (!token || !isValidTokenFormat(token))) {
  ...
  const config = {
    ...
    headers: {
      ...
      ...(token && isValidTokenFormat(token) && { Authorization: `Bearer ${token}` }),
      ...options.headers,
    },
  };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        if (currentToken === 'offline-token' || !isValidTokenFormat(currentToken)) {
          localStorage.removeItem('token');
          localStorage.removeItem('user');
        }
        
        const error = new Error(data.message || 'Invalid or expired token. Please log in again.');
        error.status = 401;
        error.data = data;
      ...
      // Create an error object with all error details
